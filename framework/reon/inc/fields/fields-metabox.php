...
add_action('reon/render-metabox-fields', 'reon_fields_metabox', 10, 3);
if (!function_exists('reon_fields_metabox')) {

    function reon_fields_metabox($fields, $metabox_id, $post_id) {

        $index = 0;
        foreach ($fields as $fld) {
            $field = $fld;
            $field['screen'] = 'metabox';
            ...
            if (isset($field['fields']) && isset($field['merge_fields']) && $field['merge_fields'] === false) {
                ...
                foreach ($field_ids as $field_id) {
                    $mt_ar = get_post_meta($post_id, $field_id, false);
                    if (count($mt_ar) == 1) {
                ...
                $meta_ar = get_post_meta($post_id, $field['id'], false);
