...
if ( !class_exists( 'WTARS_Shipped_Admin_Cost_Type_Height' ) && !defined( 'WTARS_SHIPPED_PREMIUM' ) ) {
    ...
    class WTARS_Shipped_Admin_Cost_Type_Height {
        ...
        public function get_types( $in_types, $args ) {
            ...
            /* translators: 1: cost text 2: height unit */
            $in_types[ 'prem_16' ] = sprintf( esc_html__( '%1$s per height (%2$s) (Premium)', 'table-rate-shipping-rates' ), $args[ 'format_cap_text' ], $height_unit );
            /* translators: 1: cost text 2: height unit */
            $in_types[ 'prem_17' ] = sprintf( esc_html__( '%1$s x height (%2$s) (Premium)', 'table-rate-shipping-rates' ), $args[ 'format_cap_text' ], $height_unit );
