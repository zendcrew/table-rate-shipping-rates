...
if ( !class_exists( 'WTARS_Shipped_Admin_Data_List_Checkout_Message_Methods' ) && !defined( 'WTARS_SHIPPED_PREMIUM' ) ) {
    ...
    class WTARS_Shipped_Admin_Data_List_Checkout_Message_Methods {
        ...
        public function get_data_list( $result, $data_args ) {
            ...
            $result[ 'prem_1' ] = esc_html__( 'Apply only this notification (Premium)', 'table-rate-shipping-rates' );
            $result[ 'prem_2' ] = esc_html__( 'Apply if other notifications are valid (Premium)', 'table-rate-shipping-rates' );
            $result[ 'prem_3' ] = esc_html__( 'Apply if no other valid notifications (Premium)', 'table-rate-shipping-rates' );
